...
| 14 | [Commands, Method References & Tuples](guide/14-functions-and-lambdas.md) | `command` — the third kind of member — with `carries ... into`, command types as roles, the inline form; `methodref`, `methodptr<>` for a bare code address, tuple returns, and named arguments. |
