...
## 4.1 Value semantics: assignment is a deep copy
...
## 4.2 Sharing on purpose: pointers (`T*`) and references (`T&`)
...
## 4.3 Where objects live: stack vs heap
...
- **`on stack`** places the object in the current frame. It is reclaimed automatically
  ...
  to exist at the closing brace (running its destructor on the way out; see §4.5).
...
## 4.4 No undefined behavior: dangling, double-free, and the trap
...
## 4.5 RAII: destructors and automatic cleanup
...
## 4.6 `defer` and `using`: cleanup that can't be skipped
...
**Default (no prefix) — copy semantics.** Everything in §4.1. Assignment copies; multiple
...
## 4.9 Persistents and `reattach`: state that outlives its object
