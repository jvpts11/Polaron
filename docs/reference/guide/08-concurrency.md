...
freestanding mode** (see §8.9): the compiler rejects `async`/`await` outright there, and the thread,
channel, and lock types have nothing to lower to without an OS underneath.

## 8.1 Two models, side by side
...
### 8.3.1 How an async method becomes a state machine
...
### 8.3.2 What `await` actually does
...
### 8.3.3 Awaiting inside loops and conditionals
...
### 8.3.4 Results and exceptions both propagate
...
### 8.3.5 Awaiting outside an async method
...
## 8.9 Concurrency is not available in freestanding mode
