# 9. Compile-Time, Reflection & Universal Prefixes
...
zero-runtime machinery and work everywhere, including **freestanding** mode (§14); others lean on the
managed runtime and are therefore rejected in freestanding. The chapter flags which is which at each
step, and §9.6 collects the answers in one place.
...
annotations ride along on the declaration and become visible to reflection (§9.2).
...
Used as a prefix, `comptime` is the same feature described in §9.1: it forces the thing it precedes
...
## 9.6 Availability in freestanding mode

Freestanding mode (§14) drops the managed runtime, so the features in this chapter split cleanly by
