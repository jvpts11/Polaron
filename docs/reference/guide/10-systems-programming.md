# 10. Systems Programming: Builtins, FFI & Freestanding
...
## 10.1 Compiler builtins vs. the standard library
...
## 10.2 Strings: `String` and `string`
...
(§10.7), it is the raw `data` pointer that crosses the boundary, and knowing the layout explains
why.
...
## 10.4 `Decimal`: exact fixed-point arithmetic
...
## 10.5 Low-level memory: `address`, raw pointers, and the `Memory` API
...
## 10.6 SIMD vectors: `vec2`/`vec3`/`vec4` and `mat4`
...
## 10.7 `System.IO.Console`: the managed console
...
takes away (§10.9). In hosted programs it is your everyday I/O, and it must be imported.
...
buffer yourself — which is exactly what §10.9's VGA example does.
