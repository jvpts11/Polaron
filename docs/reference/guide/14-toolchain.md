...
| `ldp3 explain <code>` | Print the why / fix / prevent for a diagnostic code (see [§12](#12-diagnostics)). |
...
- **`[build]`** — `output` (the build directory), `target` (an LLVM triple, e.g.
  `x86_64-windows`), `freestanding` (bare-metal mode; see [§10](#10-systems-programming)),
...
## 14.4 Dependencies and environments
