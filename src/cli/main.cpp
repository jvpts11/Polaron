...
constexpr std::string_view kVersion = "polc 1.0.175";
...
int compile(const std::vector<std::string>& inputs, const std::string& outPath,
    ...
    // without. Three and a half times, on the one path where latency is the whole product. It is
    // also what took `diagnostics_all_carry_a_code` from 284 seconds to past its 900-second limit --
    // that test runs polc over the corpus, and every invocation was paying for a module nobody
    // asked for and nobody would read.
    //
...
int main(int argc, char** argv) {
    ...
    if (args[0] == "--check") {
        ...
        for (std::size_t i = 1; i < args.size(); ++i) {
            if (args[i] == "--lib") {
                ...
                // Accepted and a no-op: it is how the analysis always behaves now. Kept so build
                // files written while it was opt-in keep running. There is no `--permissive` twin --
                // see the note on `outlivesOrEqual`: a partial guarantee here is not one.
    ...
    for (std::size_t i = 0; i < args.size(); ++i) {
        if (args[i] == "-o") {
            ...
            // A no-op: how the analysis always behaves. Kept so old build files keep running.
        } else if (args[i] == "--no-region-binder") {
