...
constexpr Row kCatalog[] = {
    ...
    // fork holding 0B0B, 0B0C and 0B0D -- which the structural advice on this side had already
    // taken, and had taken into the prelude ratchet, into test regexes and into `[Allow(code: ...)]`
    // annotations written in Polaron source. A diagnostic code is a name the outside world says back
    // to you; the three that nothing outside their own compiler had said yet are the three that
    // move. See `docs/reference/guide/13-diagnostics.md`, which lists them under these numbers.
