...
// `lives` answers "does this class name its own region", and it is a parameter because the answer
// lives in the code generator while the walk lives here. A region class must NOT be promoted: escape
// is what its region is for, and rewriting the placement made the emitter refuse the very line the
// author wrote. See the note at the call site in codegen_module.cpp.
