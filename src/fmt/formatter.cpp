...
bool needsSpace(const Token& prev, bool prevUnary, const Token& cur) {
    ...
    if (ambiguous(prev) || ambiguous(cur)) {
        return hadSpaceBetween(prev, cur);
    }
    if (is(cur, ";") || is(cur, ",") || is(cur, ")") || is(cur, ".")) {
        return false;
    }
    if (is(prev, ".") || is(prev, "(")) {
        return false;
    }
    if (is(cur, "(") && isValueEnd(prev)) {
        return false;  // call
    }
    if (prevUnary) {
        return false;
...
std::string format(std::string_view source, std::string_view file, bool* ok) {
    ...
    for (const Token& tok : tokens) {
        ...
        if (!haveEmitted) {  // first token: indent only, no leading newline
            ...
            if (needsSpace(prev, prevUnary, tok)) {
                out += " ";
            }
        }
