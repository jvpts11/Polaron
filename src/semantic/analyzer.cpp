...
void SemanticAnalyzer::registerConsts(const ast::Program& program) {
    auto reg = [&](const ast::ConstDecl& c, const std::string& owner) {
        ...
        // A NEWTYPE OVER A NUMBER IS A NUMBER HERE. `newtype Vector = int;` exists precisely so that
        // the vectors can be NAMED -- `fixed Vector TimerVector = cast<Vector>(32);` is the first
        // thing anyone writes with one, and rejecting it left the feature usable for parameters and
        // locals but not for the constants those parameters are called with. Classified by the
        // underlying type, exactly as casts are (analyzer_typeof.cpp), so the constant stays a
        // distinct type everywhere it is used and is merely storable here.
        const std::string underlying = [&] {
            auto it = newtypes_.find(baseType(type));
            return it != newtypes_.end() ? it->second : type;
        }();
        if (!isNumeric(underlying) && underlying != "boolean" && underlying != "char") {
