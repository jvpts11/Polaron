...
// A project found on disk: its directory (the one holding polaron.toml) and the parsed manifest.
struct DiscoveredProject {
    std::filesystem::path dir;
    Manifest manifest;
};

// Walk `root` recursively for polaron.toml files -- skipping packages/, build-output/, .git and node_modules --
// parse each, and return the projects sorted by name. Unreadable manifests are skipped. This is the engine
// used by the TUI's launch scan; a future `polaron --json` mode can expose it too.
...
// Projects the user has opened before, remembered across sessions in ~/.pol/registry.toml so the TUI can
// surface them without a full disk scan. loadRememberedProjects returns the ones whose polaron.toml still
