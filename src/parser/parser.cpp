...
ast::StmtPtr Parser::parseStatement() {
    ...
    if (check(TokenKind::KwUnimport) || check(TokenKind::KwReimport) ||
        ...
        if ((check(TokenKind::KwReimport) || check(TokenKind::KwImport)) && validated) {
            ...
            // `expecting` IS A SOFT KEYWORD: matched by text here, and an ordinary name everywhere
            // else. A hard one costs the word to every program in the world for a feature almost
            // none of them use -- and a real game wanted `expecting` for the same reason a body has
            // one (how many children are coming). See `reimport_expecting.pol`, which declares a
            // local by that name and prints it.
...
ast::ExprPtr Parser::parseUnimportExpr() {
    ...
    expectWord("expecting", "'expecting'");   // soft keyword -- see `parseStatement`
