; ModuleID = 'C:/Users/jvpts/Documents/GitHub/LDP3/tests/samples/bundle_optional_app.pol'
source_filename = "C:/Users/jvpts/Documents/GitHub/LDP3/tests/samples/bundle_optional_app.pol"
...
@.str = private unnamed_addr constant [13 x i8] c"square = %d\0A\00", align 1
...
@.str.2 = private unnamed_addr constant [20 x i8] c"calc not available\0A\00", align 1
@.strdata.1298 = private constant [18 x i8] c"bundle not loaded\00"
@.strobj.1299 = private global %String { i64 17, ptr @.strdata.1298, i64 0 }
@.strdata.1300 = private constant [20 x i8] c"bundle ABI mismatch\00"
@.strobj.1301 = private global %String { i64 19, ptr @.strdata.1300, i64 0 }
...
@__dynh_calc = internal global ptr null
@.dynpath = private unnamed_addr constant [66 x i8] c"C:/Users/jvpts/Documents/GitHub/LDP3/build2/tests/e2e_absent.polb\00", align 1
@.dynfp = private constant [32 x i8] c"\CB\1A2Y\8E\E9\E8\D7\8Fw(\90\EF\\[\EB\BD\D9Y<\14\97\8BR\C57\DDZ\95q\D25"
@.dynsym = private unnamed_addr constant [12 x i8] c"Calc.square\00", align 1
...
argv.end:                                         ; preds = %argv.cond
  ...
  %16 = invoke i32 @Calc.square(i32 7)
          to label %invoke.cont unwind label %ehpad

ehpad:                                            ; preds = %argv.end
  %17 = catchswitch within none [label %catch.dispatch] unwind to caller
...
define internal i32 @Calc.square(i32 %0) personality ptr @__CxxFrameHandler3 {
entry:
  %exc.thrown3 = alloca ptr, align 8
  %exc.thrown = alloca ptr, align 8
  %status = alloca i32, align 4
  store i32 0, ptr %status, align 4
  %h = load ptr, ptr @__dynh_calc, align 8
  %1 = icmp eq ptr %h, null
  br i1 %1, label %dyn.load, label %dyn.cont

dyn.load:                                         ; preds = %entry
  %loaded = call ptr @polaron_bundle_load(ptr @.dynpath, ptr @.dynfp, ptr %status)
  store ptr %loaded, ptr @__dynh_calc, align 8
  br label %dyn.cont

dyn.cont:                                         ; preds = %dyn.load, %entry
  %bundle = load ptr, ptr @__dynh_calc, align 8
  %sym = call ptr @polaron_bundle_sym(ptr %bundle, ptr @.dynsym)
...
entry:
  %strcpy = call ptr @__polaron_str_copy(ptr @.strobj.1301)
  ret ptr %strcpy
}
