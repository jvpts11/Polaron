...
#
# Builds a local git repo with several semver tags, then `ldp3 plug <repo>@^1.0.0` and checks the highest
...
file(WRITE "${fix}/src/lib.ldp3" "${libbody}")
...
foreach(ver 1.0.0 1.2.0 1.3.5 2.0.0)
    file(APPEND "${fix}/src/lib.ldp3" "// version ${ver}\n")
    execute_process(COMMAND git -c user.name=ci -c user.email=ci@ldp3 commit -q -a -m "v${ver}"
    ...
    if(NOT rc EQUAL 0)
        ...
        execute_process(COMMAND git -c user.name=ci -c user.email=ci@ldp3 commit -q -m "v${ver}"
            WORKING_DIRECTORY "${fix}" RESULT_VARIABLE rc)
        if(NOT rc EQUAL 0)
...
execute_process(COMMAND "${LDP3}" plug "${fix}@^1.0.0"
