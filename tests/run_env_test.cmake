...
#
# Redirects ~/.ldp3 to a throwaway LDP3_HOME so nothing touches the real home. Creates a named environment,
...
set(ENV{LDP3_HOME} "${home}")
file(REMOVE_RECURSE "${home}" "${fix}" "${app}")

# --- env new ---
execute_process(COMMAND "${LDP3}" env new gamedev RESULT_VARIABLE rc OUTPUT_VARIABLE out)
...
# --- env list / env remove ---
execute_process(COMMAND "${LDP3}" env list OUTPUT_VARIABLE out)
if(NOT out MATCHES "gamedev")
    message(FATAL_ERROR "env list did not show gamedev: [${out}]")
endif()
execute_process(COMMAND "${LDP3}" env remove gamedev)
