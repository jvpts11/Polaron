# `polaron fmt` end-to-end, invoked via `cmake -P`.
#
...
execute_process(COMMAND "${POLARON}" fmt WORKING_DIRECTORY "${app}" RESULT_VARIABLE rc OUTPUT_VARIABLE out)
message(STATUS "fmt: ${out}")
if(NOT rc EQUAL 0)
    message(FATAL_ERROR "polaron fmt failed (exit ${rc})")
endif()

file(READ "${app}/src/main.pol" f1)
...
execute_process(COMMAND "${POLARON}" fmt WORKING_DIRECTORY "${app}")
file(READ "${app}/src/main.pol" f2)
...
execute_process(COMMAND "${POLARON}" run WORKING_DIRECTORY "${app}" OUTPUT_VARIABLE runout RESULT_VARIABLE rrc)
string(REGEX REPLACE "[ \t\r\n]+" " " runout "${runout}")
if(NOT runout MATCHES "Resultado: 42")
    message(FATAL_ERROR "reformatted program did not run correctly: [${runout}]")
endif()

message(STATUS "OK: polaron fmt")
