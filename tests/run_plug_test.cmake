...
if(NOT EXISTS "${app}/libraries/mathlib/mathlib.polb")
    message(FATAL_ERROR "expected libraries/mathlib/mathlib.polb was not created")
endif()
# THE ENTRY SAYS WHAT IT IS, WHERE IT IS, AND WHERE IT CAME FROM. It used to be the URL alone, which
# named the place the library had been fetched from and said nothing about the thing now sitting in the
# project -- so a reader could not tell from the manifest what was installed or where it lived.
file(READ "${app}/polaron.toml" mf)
if(NOT mf MATCHES "mathlib = { path = \"libraries/mathlib\", source = \"[^\"]+@v1\\.0\\.0\" }")
    message(FATAL_ERROR "manifest did not record the dependency as path + source:\n${mf}")
endif()

execute_process(COMMAND "${POLARON}" unplug mathlib WORKING_DIRECTORY "${app}" RESULT_VARIABLE rc)
if(EXISTS "${app}/libraries/mathlib")
    message(FATAL_ERROR "unplug did not remove the library directory")
endif()
file(READ "${app}/polaron.toml" mf)
if(mf MATCHES "mathlib")
    message(FATAL_ERROR "unplug left the entry in the manifest:\n${mf}")
endif()
